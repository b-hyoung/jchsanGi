...
import { DEFAULT_CUSTOM_THEME_COLOR, getThemeMeta, normalizeCustomThemeColor, normalizeThemeId } from '@/lib/themeCatalog';
...
export function aggregateThemeUsage(events) {
  const all = Array.isArray(events) ? events : [];
  const changes = all.filter((event) => String(event?.type || '') === 'theme_change');
  const latestByUser = new Map();
  const themeMap = new Map();
  const customColorMap = new Map();

  changes.forEach((event) => {
    const email = String(event?.payload?.__meta?.userEmail || event?.payload?.userEmail || '')
      .trim()
      .toLowerCase();
    const clientId = String(event?.clientId || '').trim();
    const userKey = email || clientId;
    if (!userKey) return;
    const timestamp = String(event?.timestamp || '');
    const prev = latestByUser.get(userKey);
    if (!prev || timestamp >= String(prev?.timestamp || '')) {
      latestByUser.set(userKey, event);
    }
  });

  latestByUser.forEach((event) => {
    const themeId = normalizeThemeId(event?.payload?.themeId || event?.payload?.theme);
    const meta = getThemeMeta(themeId);
    const timestamp = String(event?.timestamp || '');
    const customColor = themeId === 'custom' ? normalizeCustomThemeColor(event?.payload?.customColor) : '';

    if (!themeMap.has(themeId)) {
      themeMap.set(themeId, {
        themeId,
        label: meta.label,
        swatch: themeId === 'custom' ? customColor || DEFAULT_CUSTOM_THEME_COLOR : meta.color,
        count: 0,
        clients: 0,
        lastUsedAt: '',
      });
    ...
    const row = themeMap.get(themeId);
    row.count += 1;
    row.clients += 1;
    if (timestamp && timestamp > row.lastUsedAt) row.lastUsedAt = timestamp;
    if (themeId === 'custom' && customColor) row.swatch = customColor;

    if (themeId === 'custom' && customColor) {
      if (!customColorMap.has(customColor)) {
        customColorMap.set(customColor, {
          color: customColor,
          count: 0,
          clients: 0,
          lastUsedAt: '',
      ...
      const customRow = customColorMap.get(customColor);
      customRow.count += 1;
      customRow.clients += 1;
      if (timestamp && timestamp > customRow.lastUsedAt) customRow.lastUsedAt = timestamp;
    }
  });

  const totalChanges = changes.length;
  const totalActiveClients = latestByUser.size;
  const themes = Array.from(themeMap.values())
    .map((row) => ({
      ...
      swatch: row.swatch,
      count: row.count,
      uniqueClients: row.clients,
      share: totalActiveClients > 0 ? Number(((row.count / totalActiveClients) * 100).toFixed(1)) : 0,
      lastUsedAt: row.lastUsedAt,
    }))
    .sort((a, b) => {
      if (b.count !== a.count) return b.count - a.count;
      if (b.uniqueClients !== a.uniqueClients) return b.uniqueClients - a.uniqueClients;
      return String(b.lastUsedAt || '').localeCompare(String(a.lastUsedAt || ''));
    });

  const customTotal = themeMap.get('custom')?.count || 0;
  const customColors = Array.from(customColorMap.values())
    .map((row) => ({
      color: row.color,
      count: row.count,
      uniqueClients: row.clients,
      shareWithinCustom: customTotal > 0 ? Number(((row.count / customTotal) * 100).toFixed(1)) : 0,
      lastUsedAt: row.lastUsedAt,
    }))
    .sort((a, b) => {
      if (b.count !== a.count) return b.count - a.count;
  ...
  return {
    summary: {
      totalChanges,
      uniqueClients: totalActiveClients,
      customChanges: customTotal,
      presetChanges: totalActiveClients - customTotal,
      topThemeId: themes[0]?.themeId || '',
      topThemeLabel: themes[0]?.label || '-',
...
export function aggregateMetrics(events) {
  ...
  const themeUsage = aggregateThemeUsage(all);
