...
- **present_similar_problem 필드 작성 규칙:**
  ...
  · **SQL 유사 문제 창의성 규칙 (중요):**
    - 원래 문제와 같은 SQL 키워드가 정답이 되면 안 된다. 반드시 다른 키워드가 정답이 되게 하라.
    - 예: 원래 답이 SET이면 → INSERT INTO, DELETE FROM, HAVING, GROUP BY 등 다른 DML/DDL 키워드가 답이 되는 문제를 내라.
    - 예: 원래 답이 GRANT/ON/TO이면 → REVOKE/FROM 또는 CREATE/ALTER/DROP 등 다른 DCL/DDL이 답이 되�� 하라.
    - 같은 concept(DML, DCL 등)를 테스트하되, 빈칸의 위치와 답이 되는 키워드를 완전히 바꿔라.
    - 테이블명, 컬럼명, 조건값도 원래 문제와 다르게 하라.
{problem_tags_block}
