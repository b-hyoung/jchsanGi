  ...
  return (
    <div className="min-h-screen w-full bg-white flex flex-col">
      <header className="bg-white shadow-md p-4 flex justify-between items-center">
        <h1 className="text-xl font-bold text-sky-800">{session.title}</h1>
      ...
      <footer className="bg-white shadow-t-md p-4 flex justify-between items-center">
        <button
          ...
          className="px-8 py-3 bg-gray-200 text-gray-800 font-bold rounded-lg hover:bg-gray-300 disabled:bg-gray-100 disabled:text-gray-400 disabled:cursor-not-allowed inline-flex items-center"
        >
...
function TestLobby({ session, onStart, problemCount }) {
  return (
    <div className="min-h-screen w-full flex items-center justify-center bg-gradient-to-br from-white via-sky-50 to-sky-100 p-4">
      <div className="w-full max-w-2xl text-center">
        <Link href="/test" className="inline-flex items-center text-gray-600 hover:text-sky-700 mb-8 group">
          <ArrowLeft className="w-5 h-5 mr-2 transition-transform group-hover:-translate-x-1" />
          회차 선택으로 돌아가기
        </Link>
        
        <div className="bg-white/70 backdrop-blur-sm rounded-2xl shadow-xl p-8 md:p-12 border border-gray-200/50">
          <p className="text-sky-600 font-semibold">모의시험 준비</p>
          <h1 className="text-3xl md:text-4xl font-extrabold text-sky-900 mt-2 mb-4">
            {session.title}
          </h1>
          ...
          <button
            onClick={onStart}
            className="w-full max-w-xs mx-auto px-8 py-4 bg-sky-600 text-white font-bold text-lg rounded-full hover:bg-sky-700 transition-transform transform hover:scale-105 focus:outline-none focus:ring-4 focus:ring-sky-300 inline-flex items-center justify-center"
          >
