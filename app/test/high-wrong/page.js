...
export default async function HighWrongRateQuizPage({ searchParams }) {
  ...
  const outcomeStatsMin2 = aggregateProblemWrongRatesFromOutcomes(problemOutcomes, { minAttempts: 2 });
  ...
  // 레거시 데이터(문항별 결과 저장 전)도 활용할 수 있게 이벤트 집계로 보조 처리
  if (stats.length === 0) {
    const events = await readEvents();
    const legacyStatsMin2 = aggregateProblemWrongRates(events, { minAttempts: 2 });
    usingMinAttempts = legacyStatsMin2.length > 0 ? 2 : 1;
    stats = legacyStatsMin2.length > 0 ? legacyStatsMin2 : aggregateProblemWrongRates(events, { minAttempts: 1 });
  }
  ...
  for (const row of topCandidates) {
    ...
    try {
      if (!datasetCache.has(sid)) {
        ...
        if (!ds) {
          datasetCache.set(sid, null);
        } else {
          datasetCache.set(sid, ds);
        }
      }
      ...
      // skip unreadable datasets but keep page available
    }
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 p-4">
        <div className="w-full max-w-xl rounded-2xl border border-gray-200 bg-white p-8 text-center shadow-sm">
          <h1 className="text-2xl font-extrabold text-slate-900 mb-3">오답률 높은 문제 풀기</h1>
          <p className="text-slate-600 mb-2">아직 문항별 오답률 데이터가 충분히 쌓이지 않았습니다.</p>
          <p className="text-sm text-slate-500 mb-6">
            시험 제출 시 문항별 결과가 누적되면 자동으로 구성됩니다. (오답률 집계 데이터 없음)
          </p>
  ...
  return (
    <Quiz
      ...
      session={{
        title: `오답률 높은 문제 풀기 (${picked.length}문제)`,
        reviewOnly: true,
        lobbySubtitle: `오답률 상위 문항 기준 / 최소 ${usingMinAttempts}회 시도 / 총 ${picked.length}문항`,
      }}
      sessionId="high-wrong"
