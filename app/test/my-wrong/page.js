...
import { getUserWrongProblems } from '@/lib/userProblemsStore';
...
export default async function MyWrongQuizPage({ searchParams }) {
  ...
  const wrongList = await getUserWrongProblems(userEmail);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 p-4">
        <div className="w-full max-w-xl rounded-2xl border border-gray-200 bg-white p-8 text-center shadow-sm">
          <h1 className="text-2xl font-extrabold text-slate-900 mb-3">내 틀린 문제 풀기</h1>
          <p className="text-slate-600 mb-6">아직 틀린 문제 기록이 없습니다. 시험을 완료하면 자동으로 쌓입니다.</p>
  ...
  return (
    <Quiz
      ...
      session={{
        title: `내 틀린 문제 풀기 (${picked.length}문제)`,
        reviewOnly: true,
        lobbySubtitle: `${session.user.name || userEmail} 님의 오답 문제 모음 / 총 ${picked.length}문항`,
      }}
      sessionId="my-wrong"
