...
- 관리자 리포트 대시보드 (`/admin`)
  ...
  - GPT 이의신청 캐시 조회 탭 (`/admin`)
    - GPT 재호출 없이 Supabase 캐시만 조회(운영 검토용)
    - 과목별(1~3과목) 캐시 행 수/조회수 요약
    - 회차/문항 기준 `가장 많이 조회된 문제` 집계(top problems)
    - 캐시 상세 목록 페이지네이션(10/20/50/100)
    - 정렬(오름/내림): 과목/회차/문항/조회수/생성시각
    - 피드백 필터: `평가 있음 / 좋아요 우세 / 싫어요 우세 / 좋아요 있음 / 싫어요 있음`
    - 캐시 상세 모달: 사용자 질문/선택답/정답/GPT 캐시 답변/문제 이동 링크 확인
